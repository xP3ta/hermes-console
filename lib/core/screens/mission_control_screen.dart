...
/// Bot Chat now respects the caller's connection.readOnly flag (and upstream
/// capability/ownership/server gates). Writable owned connections expose the
/// composer; read-only callers or blocked cases stay read-only.
/// Name kept for call-site/test compatibility during the surface-level fix.
@visibleForTesting
...
class _MissionControlScreenState extends State<MissionControlScreen>
  ...
  /// Opens the agent's canonical Bot Chat using the actual connection (writable
  /// for owned non-readOnly connections; read-only preserved for callers/gates).
  Future<void> _openChat(MissionAgent agent) async {
    final snapshot = _snapshot;
    if (snapshot == null ||
        snapshot.profilesCapability != MissionCapabilityState.available) {
      // The retained roster is browse-only while capability data is stale or
      // unavailable. Do not clear/migrate a local pin or open a writable chat
      // from a snapshot that is no longer authoritative.
